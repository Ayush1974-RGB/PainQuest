...
type FeedbackMap  = Record<ExerciseType, { up: string; down: string; mid: string }>;
...
function getAngle(pose: PoseResult, exercise: ExerciseType): { angle: number; confidence: number } {
  const tryLeft = (a: string, b: string, c: string) => {
    const A = getKP(pose, `left_${a}`);
    const B = getKP(pose, `left_${b}`);
    const C = getKP(pose, `left_${c}`);
    if (A && B && C) return { angle: calculateAngle(A, B, C), confidence: Math.min(A.score, B.score, C.score) };
    return null;
  };
...
function detectFormErrors(
  ...
  switch (exercise) {
    case "bicep_curl": {
      ...
      const rEl = kp("right_elbow");
      const rWr = kp("right_wrist");
      const lSh = kp("left_shoulder");
      const lEl = kp("left_elbow");
      if (rSh && rEl) {
        const drift   = Math.abs(rEl.x - rSh.x);
        const refW    = Math.abs((lSh?.x ?? rSh.x) - rSh.x) || 100;
        if (drift > refW * 0.35) errors.push("Elbow drifting — keep it pinned to your side");
      }
      if (lSh && lEl) {
        const drift = Math.abs(lEl.x - lSh.x);
        const refW  = Math.abs((kp("right_shoulder")?.x ?? lSh.x) - lSh.x) || 100;
        if (drift > refW * 0.35) errors.push("Left elbow drifting — keep it pinned");
      }
      if (phase === "down" && rSh && rEl && rWr) {
        if (calculateAngle(rSh, rEl, rWr) < 140) errors.push("Not fully extending at the bottom — lower all the way");
      }
      break;
    }
    case "squat": {
      ...
      if (lKnee && rKnee && lAnk && rAnk) {
        ...
        const ankleW = Math.abs(lAnk.x - rAnk.x);
        if (phase === "down" && kneeW < ankleW * 0.75)
    ...
    case "pushup": {
      ...
      if (lSh && lHip && lAnk) {
        ...
        if (lHip.y > mid + 30) errors.push("Hips sagging — engage your core and keep body straight");
        if (lHip.y < mid - 40) errors.push("Hips too high — lower them to form a straight line");
      }
      if (phase === "down" && rSh && rEl && rWr) {
        if (calculateAngle(rSh, rEl, rWr) > 100) errors.push("Not going low enough — chest should nearly touch the ground");
      }
      break;
    }
    case "shoulder_press": {
      ...
      if (phase === "up" && rSh && rEl && rWr) {
        if (calculateAngle(rSh, rEl, rWr) < 155) errors.push("Not locking out — fully extend arms overhead");
      }
      if (phase === "down" && rEl && rWr) {
        if (Math.abs(rWr.x - rEl.x) > 40) errors.push("Wrists not stacked over elbows — straighten your wrist");
      }
      break;
    }
    case "jumping_jack": {
      ...
      if (phase === "up") {
        ...
        if (lAnk && rAnk && lHip && rHip) {
          ...
          const ankleW = Math.abs(lAnk.x - rAnk.x);
          if (ankleW < hipW * 1.2) errors.push("Legs not spreading wide enough — jump feet further apart");
        }
      }
      break;
    }
...
export function useExerciseAnalyzer(exercise: ExerciseType) {
  ...
  const analyze = useCallback((pose: PoseResult) => {
    ...
    const formErrors   = phase !== "neutral" ? detectFormErrors(pose, exercise, phase) : [];
    const feedback     = formErrors.length > 0 ? formErrors[0] : (phase === "up" ? fb.up : phase === "down" ? fb.down : fb.mid);
    const feedbackType = formErrors.length > 0 ? "warn" : (phase === "up" || phase === "down" ? "good" : "info");

    setState({
      reps: repsRef.current, phase,
      angle: Math.round(angle), feedback, feedbackType, confidence, formErrors,
    });
  }, [exercise]);
